(() => {
  ...
  // ── GET COMPANY FROM BADGE LINK NEXT TO NAME ──────────────────────────────
  // This is the most reliable signal — always the current employer
  // Must be called BEFORE headline extraction so we don't use headline for company
  function getCompanyFromBadge(name) {
    const h1 = document.querySelector("h1");
    // Search in progressively wider containers around h1
    const containers = [
      h1?.parentElement,
    ...
    for (const area of containers) {
      const links = Array.from(area.querySelectorAll('a[href*="/company/"]'));
      for (const a of links) {
        // Skip promoted / ad / sidebar content
        let skip = false;
        let node = a;
        for (let i = 0; i < 8 && node; i++) {
          const cls = (node.className||"").toString().toLowerCase();
          const dataView = (node.getAttribute?.("data-view-name")||"").toLowerCase();
          if (cls.includes("promoted")||cls.includes("sponsor")||cls.includes("pymk")||
              cls.includes("people-also")||cls.includes("similar-profile")||
              dataView.includes("promoted")||node.getAttribute?.("data-ad-banner")) {
            skip=true; break;
          }
        ...
        // Stop at Experience section — only want intro card company
        ...
        if (lbl && lbl.length > 1 && lbl.length < 80 && lbl !== name) {
          console.log("[Hirely] company from badge:", lbl);
          return lbl;
        }
      }
  ...
  // Company is already known from badge, so just extract the title part
  function getTitleFromHeadline(headline, knownCompany) {
    if (!headline) return headline;

    // "Title at Company" — take only title part
    if (headline.includes(" at ")) {
      const m = headline.match(/^(.+?)\s+at\s+/i);
      if (m) return m[1].trim();
    }

    // "Title @ Company" — @ symbol variant
    if (headline.includes(" @ ")) {
      return headline.split(" @ ")[0].trim();
    }

    // "Title | Segment2 | Segment3" — take first segment
    if (headline.includes(" | ")) {
      return headline.split(" | ")[0].trim();
    }

    // "Title, Company" — if company is already known, strip it
    if (headline.includes(",") && knownCompany) {
      const ci = headline.indexOf(",");
      const titlePart = headline.slice(0, ci).trim();
      if (titlePart.length > 3 && titlePart.length < 100) return titlePart;
    }

    // Return full headline as title if no pattern matched
    return headline;
  }

  // ── MAIN SCRAPE ───────────────────────────────────────────────────────────
  function scrapeProfile() {
    // NAME from document.title (server-rendered, most reliable)
    function parseTitle(raw) {
      ...
      // Name is first segment, strip credentials like "B.A." ", CHRP" etc
      const rawName = segs[0].split(",")[0].trim();
      return { name: rawName, headline: segs[1]||"" };
    }

    const titleData = parseTitle(getMeta("og:title"))||parseTitle(document.title);
    let name = titleData?.name||"";
    let rawHeadline = titleData?.headline||"";
    ...
    // ── STEP 1: Get company from badge link FIRST (most reliable) ─────────
    let company = getCompanyFromBadge(name);

    // ── STEP 2: Get headline from DOM if document.title had none ──────────
    if (!rawHeadline) {
      const h1 = document.querySelector("h1");
      if (h1 && name) {
        const JUNK = /^(connect|message|follow|pending|she\/her|he\/him|they\/them|\d+\s*(connections?|followers?)|contact info|share|profile enhanced|open to work|hiring|save in sales|more|pending|open to|add section|add custom)/i;
        const LOCATION = /^(macomb|detroit|toronto|etobicoke|greater|united states|canada|michigan|ontario|california|texas|florida|ohio|washington|new york|los angeles|london|vietnam|india|australia)\b/i;
        ...
        while (node && candidates.length < 40) {
          const t = (node.textContent||"").trim();
          if (t.length>5&&t.length<250&&!seen.has(t)&&!JUNK.test(t)&&!LOCATION.test(t)) {
            seen.add(t);
            candidates.push(t);
          }
        ...
        // Pick first candidate that:
        // 1. Is not a name variant (including with pronouns, credentials, parenthetical nicknames)
        // 2. Is not a location
        // 3. Contains job-related content
        const firstName = name.split(" ")[0];
        const lastName = name.split(" ").slice(-1)[0];

        for (const c of candidates) {
          // Skip if starts with person's first or last name (name variants)
          if (c.startsWith(firstName)&&c.length<name.length+30) continue;
          if (c.startsWith(lastName)&&c.length<30) continue;
          // Skip pure credentials
          if (/^(b\.?a\.?|m\.?b\.?a\.?|ph\.?d\.?|chrp|pmp|cpa|cfa|rn|md|jd|llb)\.?$/i.test(c)) continue;
          // Skip if it looks exactly like a name (2-4 capitalized words, no job words)
          if (/^[A-Z][a-zA-Z\(\)]+(\s+[A-Z][a-zA-Z\(\)]+){1,3}$/.test(c)&&
              !c.includes(" at ")&&!c.includes(",")&&!c.includes("|")&&!c.includes("@")&&
              !/\b(manager|director|founder|engineer|analyst|coordinator|specialist|consultant|recruiter|developer|designer|lead|head|senior|junior|associate|partner|president|officer|executive|advisor|coach|tutor|generalist|business|human|resources|talent|acquisition|operations)\b/i.test(c)) continue;
    ...
    // ── STEP 3: Extract title from headline (company already known) ────────
    const displayTitle = getTitleFromHeadline(rawHeadline, company);

    // ── STEP 4: If still no company, try extracting from headline ─────────
    if (!company && rawHeadline) {
      if (rawHeadline.includes(" at ")) {
        const m = rawHeadline.match(/\s+at\s+(.+)$/i);
        if (m) company = m[1].trim().split("|")[0].split(",")[0].trim();
      } else if (rawHeadline.includes(" @ ")) {
        company = rawHeadline.split(" @ ")[1]?.split("|")[0].trim()||"";
      }
    }
