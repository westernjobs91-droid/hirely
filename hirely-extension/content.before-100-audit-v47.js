/* Hirely Capture v4.7 — complete content.js replacement.
...
var HirelyEngine = (() => {
  ...
  function selectCurrentRole(active, badge, headline) {
    ...
    if (!pool.every(p => same(p.company, pool[0].company))) return null;
  ...
  function scrapeProfile(doc, pageUrl) {
    ...
    const selected = selectCurrentRole(active, badge, result.headline);
    if (selected) {
  ...
  return {findProfileHeading, findExperience, scrapeProfile, parseExperience, splitTitleAndCompany, stripDegree, splitName, selectCurrentRole, isPronounOrDegree, textOf, emptyResult, fieldsPass, companyQuality: s => validLabel(s) ? 3 : 0, version:'4.7'};
})();
...
(() => {
  ...
  async function render() {
    ...
    panel.innerHTML=
      '<div class="hirely-header"><div class="hirely-header-title"><div class="hirely-header-logo">H</div><span>Hirely v4.7</span></div>'+
    ...
    try {
      ...
      panel.innerHTML='<div class="hirely-header"><div class="hirely-header-title"><div class="hirely-header-logo">H</div><span>Hirely v4.7</span></div>'+
  ...
  function renderRoleOptions(state) {
    ...
    if (slot.dataset.roles !== signature) {
      ...
      roles.forEach((r,i) => select.appendChild(new Option(r.title + ' — ' + r.company + ' (' + r.dates + ')', String(i))));
  ...
  async function renderSaveTab(container,session,data,existing) {
    ...
    const sourceTag=data.enriched
      ...
      :'<div class="hirely-source-tag">Scraped from LinkedIn · v4.7</div>';
    ...
    if(existing){
      ...
      container.innerHTML=sourceTag+
        ...
        (data.email?'<div class="hirely-email-row">&#x2709; <span class="hirely-email-val">'+escapeHtml(data.email)+'</span><button class="hirely-copy-btn" data-copy="'+escapeAttr(data.email)+'">Copy</button></div>':"")+
        '<div class="hirely-role-options"></div>'+
