    ...
    <div className="space-y-3">
      <div className="bg-white border border-slate-100 rounded-2xl p-3 shadow-sm space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select aria-label="Filter by company" value={company} onChange={e => setCompany(e.target.value)} className="min-w-44 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600 outline-none focus:border-blue-400">
            <option value="all">All companies</option>{companies.map(name => <option key={name} value={name}>{name}</option>)}
          </select>
          <select aria-label="Filter by email status" value={emailStatus} onChange={e => setEmailStatus(e.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600 outline-none focus:border-blue-400">
          ...
          <select aria-label="Sort contacts" value={sort} onChange={e => setSort(e.target.value as SortMode)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600 outline-none focus:border-blue-400">
