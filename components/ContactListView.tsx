...
interface ContactListViewProps {
  ...
  emptyMessage: string
  mode?: 'contacts' | 'followups'
}

type SavedView = 'all' | 'missing-email' | 'due' | 'overdue' | 'stale' | 'replied' | 'meetings'
...
export default function ContactListView({ contacts, selectedId, onSelect, onDelete, onMove, onFindEmail, emptyMessage, mode = 'contacts' }: ContactListViewProps) {
  ...
  const visible = useMemo(() => {
    const filtered = contacts.filter(contact => {
      ...
      if (view === 'overdue' && contact.status !== 'overdue') return false
  ...
  if (contacts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 rounded-3xl border border-dashed border-slate-200 bg-white px-6 py-20 text-center shadow-sm">
        ...
        <p className="text-base font-bold text-slate-800">No contacts to show</p>
        <p className="max-w-sm text-sm leading-6 text-slate-500">{emptyMessage}</p>
      </div>
    )
  }

  const contactViews: { id: SavedView; label: string; count: number }[] = [
  ...
  const followupViews: { id: SavedView; label: string; count: number }[] = [
    { id: 'all', label: 'All due', count: contacts.length },
    { id: 'overdue', label: 'Overdue', count: contacts.filter(c => c.status === 'overdue').length },
    { id: 'missing-email', label: 'Needs email', count: contacts.filter(c => !c.email).length },
    { id: 'replied', label: 'Replied', count: contacts.filter(c => ['replied', 'meeting-set'].includes(c.status)).length },
    { id: 'meetings', label: 'Meetings', count: contacts.filter(c => c.status === 'meeting-set').length },
  ]
  const views = mode === 'followups' ? followupViews : contactViews
    ...
    <div className="space-y-3">
      ...
      <div className="hidden overflow-x-auto rounded-2xl border border-slate-200/80 bg-white shadow-sm md:block">
        <div className="min-w-[760px]">
          <div className="grid grid-cols-[28px_minmax(150px,2fr)_minmax(100px,1.15fr)_minmax(155px,1.65fr)_minmax(92px,1fr)_28px_28px] items-center gap-3 border-b border-slate-100 bg-slate-50 px-4 py-3">
            ...
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Name</span><span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Company</span><span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Email</span><span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Follow-up</span><span /><span />
          </div>
          ...
          <div className="divide-y divide-slate-50">
            {visible.map(contact => {
              ...
              return (
                <div key={contact.id} onClick={() => onSelect(contact)} className={`group relative grid grid-cols-[28px_minmax(150px,2fr)_minmax(100px,1.15fr)_minmax(155px,1.65fr)_minmax(92px,1fr)_28px_28px] items-center gap-3 px-4 py-3.5 cursor-pointer transition-all ${isOpen ? 'bg-blue-50 border-l-2 border-l-blue-500' : 'hover:bg-slate-50 border-l-2 border-l-transparent'}`}>
                  ...
                  <div className="flex min-w-0 items-center gap-3"><div className="w-8 h-8 rounded-xl flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0 shadow-sm overflow-hidden" style={{ background: contact.avatarColor }}><ContactPhoto url={contact.photoUrl} initials={initials} name={`${contact.firstName} ${contact.lastName}`} /></div><div className="min-w-0"><p className="text-[12.5px] font-semibold text-slate-900 truncate leading-tight">{contact.firstName} {contact.lastName}</p><p className="text-[10.5px] text-slate-400 truncate leading-tight mt-0.5">{contact.jobTitle || '-'}</p></div></div>
                  <p className="text-[12px] text-slate-600 truncate font-medium">{contact.company || '-'}</p>
                  {contact.email ? <div className="min-w-0"><div className="flex min-w-0 items-center gap-1.5"><svg className="w-3 h-3 text-emerald-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg><p className="truncate text-[11.5px] text-slate-600">{contact.email}</p></div><div className="mt-1"><EmailStatusBadge contact={contact}/></div></div> : <button disabled={!onFindEmail || !!finding} onClick={e => { e.stopPropagation(); findEmail(contact) }} className="w-fit rounded-lg bg-blue-50 px-2.5 py-1.5 text-[10.5px] font-semibold text-blue-700 hover:bg-blue-100 disabled:opacity-50">{finding === contact.id ? 'Searching…' : 'Find email · 1 credit'}</button>}
