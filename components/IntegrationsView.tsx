...
export default function IntegrationsView() {
  ...
  return <div className="mx-auto max-w-[1440px] space-y-5 overflow-x-hidden px-3 py-4 sm:px-6 sm:py-6">
      ...
      <div className="flex flex-col gap-5 p-5 sm:p-7 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-start gap-3 sm:gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Save people while you work in LinkedIn, Gmail, or Outlook, then manage every follow-up from one pipeline.</p>
          </div>
        </div>
    ...
    <section className="grid gap-3 sm:grid-cols-3" aria-label="Email capture workflow">
      {[
        ['1', 'Open an email', 'Choose a message in Gmail or Outlook and open Hirely.'],
    ...
    <section className="grid gap-5 lg:grid-cols-3">
      <article className="flex flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex flex-1 flex-col p-5 sm:p-6">
          ...
          <div className="mt-auto pt-6"><a href="/downloads/hirely-extension-1.4.0.zip" download className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-bold text-white shadow-sm transition hover:bg-blue-700"><svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-1m-4-4-4 4m0 0-4-4m4 4V4" /></svg>Download Chrome extension</a><p className="mt-2 text-center text-[10px] leading-4 text-slate-400">Beta install through Chrome’s Extensions page.</p></div>
        </div>
      </article>

      <article className="flex flex-col overflow-hidden rounded-3xl border border-red-100 bg-white shadow-sm">
        <div className="h-1.5 bg-gradient-to-r from-red-500 via-amber-400 to-emerald-500" />
        <div className="flex flex-1 flex-col p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-red-100 bg-red-50 text-lg font-black text-red-600">M</div><div><h3 className="text-base font-black text-slate-950">Gmail capture</h3><p className="text-xs text-slate-500">Open-message contact capture</p></div></div>
            <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700">All plans</span>
          </div>
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-slate-950 text-white shadow-sm">
      <div className="grid lg:grid-cols-[.8fr_1.2fr]">
        <div className="p-5 sm:p-7"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-blue-300">Plan access</p><h3 className="mt-2 text-xl font-black">Choose by pipeline size</h3><p className="mt-2 text-sm leading-6 text-slate-300">LinkedIn and Gmail capture are included on every plan. Outlook capture is included with Solo and Pro. Capturing a contact does not use an email-search credit.</p><a href="/pricing" className="mt-4 inline-flex text-sm font-bold text-blue-300 hover:text-blue-200">View full plan details →</a></div>
        <div className="grid gap-px bg-slate-800 sm:grid-cols-3">{[
          ['Free', '$0', 'LinkedIn + Gmail', '50 contacts'],
          ['Solo', '$29', 'LinkedIn + Gmail + Outlook', '1,000 contacts'],
          ['Pro', '$59', 'LinkedIn + Gmail + Outlook', 'Unlimited contacts'],
