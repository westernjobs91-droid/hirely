...
export default function ImportModal({ onClose, onOpenIntegrations }: ImportModalProps) {
  const chooseSource = (available: boolean) => {
    if (!available) return
    onClose()
    onOpenIntegrations()
  }
      ...
      <div className="bg-white rounded-xl border border-gray-100 w-[420px] max-w-[96vw] shadow-lg">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h2 id="import-modal-title" className="text-sm font-medium text-gray-900">Import contacts</h2>
          </div>
          <button onClick={onClose} aria-label="Close import contacts" className="text-gray-400 hover:text-gray-600 p-1 rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
        ...
        <div className="p-5">
          <div className="grid grid-cols-2 gap-3 mb-4">
            {sources.map((src, i) => (
              <button key={i} onClick={() => chooseSource(src.available)} disabled={!src.available}
                aria-label={src.available ? `${src.label}: open setup` : `${src.label}: coming soon`}
                className="border border-gray-100 rounded-lg p-4 text-center transition-all text-left enabled:hover:border-gray-200 enabled:hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-55 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
        ...
        <div className="flex justify-end px-5 py-3.5 border-t border-gray-100">
          <button onClick={onClose} className="px-4 py-2 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">Close</button>
        </div>
      </div>
    </div>
  )
