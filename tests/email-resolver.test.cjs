...
function harness({user=true,contact=null,cache=null,company=null,reserved=true}={}){
 const writes=[],calls=[];let fetched=0;
 ...
 }});return chain},async rpc(){return {data:reserved,error:null}}};
 ...
 return{run,writes,calls,get fetched(){return fetched}};
}
const person={firstName:'Jane',lastName:'Smith',company:'Example'};
...
test('free prediction uses company pattern and no paid request',async()=>{const h=harness({company:[{data:{domain:'example.com',pattern:'{first}.{last}'}}]});const d=await(await h.run(person)).json();assert.equal(d.email,'jane.smith@example.com');assert.equal(d.emailStatus,'predicted');assert.equal(d.confidence,null);assert.equal(h.fetched,0)});
test('free miss does not fall through to Hunter',async()=>{const h=harness();const d=await(await h.run(person)).json();assert.equal(d.needsPaidLookup,true);assert.equal(h.fetched,0)});
test('paid lookup needs explicit opt-in and quota',async()=>{const h=harness({reserved:false});assert.equal((await h.run({...person,action:'find'})).status,400);assert.equal((await h.run({...person,action:'find',allowPaid:true})).status,402);assert.equal(h.fetched,0)});
