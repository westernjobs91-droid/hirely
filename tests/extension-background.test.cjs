...
test('extension refreshes an expired session before a credit search',async()=>{const h=setup({expired:true});const r=await h.run({type:'HIRELY_FIND_EMAIL',action:'find',allowPaid:true,firstName:'Jane',company:'Example'});assert.equal(h.requests.length,2);assert.equal(h.requests[1].options.headers.Authorization,'Bearer fresh');const b=JSON.parse(h.requests[1].options.body);assert.equal(b.action,'find');assert.equal(b.allowPaid,true);assert.equal(r.message,'No saved company pattern.')});
...
test('failed refresh prevents email lookup',async()=>{const h=setup({expired:true,refreshOK:false});const r=await h.run({type:'HIRELY_FIND_EMAIL',action:'find',allowPaid:true});assert.equal(r.error,'NOT_LOGGED_IN');assert.equal(h.requests.length,1)});
